...
export async function POST(request: NextRequest) {
  try {
    ...
    const folderId = `poster_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    // Run face swap
    const result = await runFaceSwap(userImagePath, templatePath, outputDir, 'poster')
    ...
    console.error('Poster face swap error:', error)
    return NextResponse.json({ 
      success: false,
